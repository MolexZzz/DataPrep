...
def compute_ips(datamiss, indicator, num=20, method='lr', observed_num=1, complete_sample="Random"):  # 计算IPS值
    ...
    for i in range(p):
        if (indicator[:, i] == 0.5).any():  # 第i列存在缺失数据
            ...
            if method == 'xgb':
                ...
                combined = np.concatenate([LR_model.coef_, LR_model.intercept_.reshape(1, -1)], axis=1)
    ...
    final_array = np.vstack(combined_arrays)
    return ips, final_array