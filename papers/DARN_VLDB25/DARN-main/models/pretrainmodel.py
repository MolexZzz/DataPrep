...
class SAINT(nn.Module):
    def __init__(
        ...
        elif attentiontype in ['miss']:
            self.transformer = missTransformer(
                num_tokens=self.total_tokens,
                dim=dim,
                depth=depth,
                heads=heads,
        ...
        self.ips_simple_MLP = nn.ModuleList(
            [simple_MLP([1, 100, self.dim]) for _ in range(self.num_continuous + self.num_categories)])
