...
def p_rule(y_pred: np.ndarray, sensitive_attribute: np.ndarray, threshold: float = 0.5) -> float:
    ...
    return np.nanmin([odds, 1 / (odds + 1e-8) ])
...
def fnr_parity(y_pred, y_gt, sensitive_attribute, threshold = 0.5):
    y_pred = y_pred.ravel()
    y_gt = y_gt.ravel()
    sensitive_attribute = sensitive_attribute.ravel()
    y_pred_all = y_pred.copy()
    ...
    # Sensitive attribute = 0, y_pred = 0, y_gt = 1
    y_z_0 = y_pred[sensitive_attribute == 0]< threshold if threshold else y_pred[sensitive_attribute == 0]
    # Sensitive attribute = 1, y_pred = 0, y_gt = 1
    y_z_1 = y_pred[sensitive_attribute == 1] < threshold if threshold else y_pred[sensitive_attribute == 1]
    ...
    if len(y_z_1) == 0 or len(y_z_0) == 0:
        return 0
    parity = abs(y_z_0.mean() - y_z_1.mean())
    return parity

def fpr_parity(y_pred, y_gt, sensitive_attribute, threshold = 0.5):
    y_pred = y_pred.ravel()
    y_gt = y_gt.ravel()
    sensitive_attribute = sensitive_attribute.ravel()
    y_pred_all = y_pred.copy()
    ...
    # Sensitive attribute = 0, y_pred = 1, y_gt = 0
    y_z_0 = y_pred[sensitive_attribute == 0] > threshold if threshold else y_pred[sensitive_attribute == 0]
    # Sensitive attribute = 1, y_pred = 1, y_gt = 0
    y_z_1 = y_pred[sensitive_attribute == 1] > threshold if threshold else y_pred[sensitive_attribute == 1]    
...
def metric_evaluation(y_gt, y_pre, s, s_pre=None, prefix=""):
    ...
    acc = metrics.accuracy_score(y_gt, y_pre > 0.5)
    ap = metrics.average_precision_score(y_gt, y_pre)
    precision = metrics.precision_score(y_gt, y_pre > 0.5, zero_division=0)
    recall = metrics.recall_score(y_gt, y_pre > 0.5)
    f1 = metrics.f1_score(y_gt, y_pre > 0.5)
    auc = metrics.roc_auc_score(y_gt, y_pre)
    ...
    # avg_alpha, af_list = alpha_fairness_metric( y_pre, y_gt, s, s_pre, sample_n=10, threshold=None )
     ...
     # fnr
    fnr = fnr_parity(y_pre, y_gt, s, threshold = None)
    # fpr
    fpr = fpr_parity(y_pre, y_gt, s, threshold = None)
    # Store metric names and values in a dictionary
    # metric_name = ["acc", "ap", "precision", "recall", "f1", "auc", "dp", "dpe", "aucp", "eopp", "eoppe", "eodd", "eodde", "prule", "prulee", "abcc", "accp", "ppv", "bfp", "bfn", "fnr", "fpr"]
    # metric_name = [prefix + "/" + x for x in metric_name]
    # metric_val = [acc, ap, precision, recall, f1, auc, dp, dpe, aucp, eopp, eoppe, eodd, eodde, prule, prulee, abcc, accp, ppv, bfp, bfn, fnr, fpr]

    # metric_name = ["acc", "ap", "precision", "recall", "f1", "auc", "dp", "aucp", "eopp", "eodd", "prule",  "abcc", "accp", "ppv", "bfp", "bfn", "fnr", "fpr"]

    # metric_name = ["acc", "f1", "auc", "dp", "eopp", "eodd", "prule", "accp", "fnr", "fpr"]
    metric_name = ["acc", "f1", "auc", "dp", "eopp", "eodd", "accp", "fnr", "fpr"]

    metric_name = [prefix + "/" + x for x in metric_name]
    # metric_val = [acc, ap, precision, recall, f1, auc, dp, aucp, eopp, eodd, prule, abcc, accp, ppv, bfp, bfn, fnr, fpr]
    metric_val = [acc, f1, auc, dp, eopp, eodd, accp, fnr, fpr]
...
#     """
#     # Convert predicted probabilities to binary predictions using the threshold.
#     y_z_1 = (y_pred[sensitive_attribute == 1] > threshold).float() if threshold else y_pred[sensitive_attribute == 1]
#     y_z_0 = (y_pred[sensitive_attribute == 0] > threshold).float() if threshold else y_pred[sensitive_attribute == 0]
...
#     # Calculate the percentage difference in ground truth for each sensitive attribute value
#     y_z_1 = (y_gt[sensitive_attribute == 1] == 1).float()  # y_gt where sensitive_attribute == 1 and y_gt == 1
#     y_z_0 = (y_gt[sensitive_attribute == 0] == 1).float()  # y_gt where sensitive_attribute == 0 and y_gt == 1
